...
function Scheduler({
  ...
  const getScheduleDate = () => {
    ...
    if (isValid) {
      return (
        <span className="ml-2 mt-2 text-center w-full font-medium">{`${scheduleDay}/${scheduleMonth}/${scheduleYear} a las ${scheduleHour}:${scheduleMinute} hrs.`}</span>
      );
    }
    return (
      <span className="ml-2 mt-2 font-medium  text-center w-full text-red-700">{`${scheduleDay}/${scheduleMonth}/${scheduleYear} a las ${scheduleHour}:${scheduleMinute} hrs.`}</span>
    );
  };
  ...
  return (
    <div className="text-xs py-2">
      ...
      {enabled && (
        <div className="flex flex-col border-1 border-gray-100 rounded-lg p-4 transition ease-in-out delay-200 ">
          {showTitle && (
            <span className="flex justify-center font-medium uppercase mb-4">
              Programación
            </span>
          )}
          <div className="flex flex-row justify-start flex-wrap">
            <div className="w-40 m-1">
              <Select
                ...
                showTitle={true}
                options={months}
                selectedOption={months.find((m) => m.id === scheduleMonth)}
                onOptionChanged={(e) => {
                  setSchedule("month", e);
                }}
              ></Select>
            ...
            <div className="w-24 m-1">
              <Select
                ...
                showTitle={true}
                options={days}
                selectedOption={days.find((d) => d.id === scheduleDay)}
                onOptionChanged={(e) => {
                  setSchedule("day", e);
                }}
              ></Select>
            ...
            <div className="w-32 m-1">
              <Select
                ...
                showTitle={true}
                options={years}
                selectedOption={years.find((y) => y.id === scheduleYear)}
                onOptionChanged={(e) => {
                  setSchedule("year", e);
                }}
              ></Select>
          ...
          <div className="flex flex-row justify-start flex-wrap">
            <div className="w-24 m-1">
              <Select
                ...
                showTitle={true}
                options={hours}
                selectedOption={hours.find((h) => h.id == scheduleHour)}
                onOptionChanged={(e) => {
                  setSchedule("hour", e);
                }}
              ></Select>
            ...
            <div className="w-24 m-1">
              <Select
                ...
                showTitle={true}
                options={minutes}
                selectedOption={minutes.find((m) => m.id === scheduleMinute)}
                onOptionChanged={(e) => {
                  setSchedule("minute", e);
                }}
              ></Select>
            </div>
            <br></br>
          </div>
          <span className="mt-2">{`Esta publicación quedará disponible el día:`}</span>
          {getScheduleDate()}
          <span className=" w-full text-center font-medium">
            (-03:00) Zona horaria de Perú
          </span>
        </div>
      )}
    </div>
